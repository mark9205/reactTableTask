...
export default function TableForm({ filterSubmit, onReset }) {
	...
	return (
		<form className="table-form" onSubmit={handleSubmit}>
			<select name="name" value={name} onChange={handleNameChange} required className='select'>
				<option value="">Поле...</option>
				<option value="name">Название</option>
				<option value="points">Количество</option>
				<option value="distance">Расстояние</option>
			</select>
			<select name="law" value={law} onChange={handleLawChange} required className='select'>
