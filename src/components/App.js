	...
	useEffect(() => {
		...
		if (sortConfig.sortBox === "points") {
			sortConfig.sortDirection === "ASC"
				...
				: setSortData([
						...sortData.sort((a, b) => b.points - a.points),
				  ]);
		}
		if (sortConfig.sortBox === "distance") {
			sortConfig.sortDirection === "ASC"
				...
				: setSortData([
						...sortData.sort((a, b) => b.distance - a.distance),
				  ]);
		}
		if (
			...
			if (sortConfig.filterBox === "name") {
				if (sortConfig.filterLaw === "equal")
					setSortData([
						...sortData.filter(
							(e) => e.name === sortConfig.filterArgument
						),
					]);
			...
			if (sortConfig.filterBox === "points") {
				if (sortConfig.filterLaw === "equal")
					setSortData([
						...sortData.filter(
							(e) =>
								e.points === Number(sortConfig.filterArgument)
						),
					]);
				if (sortConfig.filterLaw === "contain")
					setSortData([
						...sortData.filter((e) =>
							e.points
								.toString()
								.includes(sortConfig.filterArgument)
						),
					]);
				if (sortConfig.filterLaw === "greater")
					setSortData([
			...
			if (sortConfig.filterBox === "distance") {
				if (sortConfig.filterLaw === "equal")
					setSortData([
						...sortData.filter(
							(e) =>
								e.distance === Number(sortConfig.filterArgument)
						),
					]);
				if (sortConfig.filterLaw === "contain")
					setSortData([
		...
		<div className="root">
			<div className="page">
				<h1 className="page__title">
					A table in the format of a single-page application as part
					of a test task
				</h1>
